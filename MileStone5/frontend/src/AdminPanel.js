...
function AdminPanel() {
  ...
  const fetchRules = async () => {
    try {
      const res = await axios.get("http://localhost:5004/regex-rules");
  ...
  const handleAddRule = async () => {
    ...
    try {
      ...
      const res = await axios.post("http://localhost:5004/regex-rules", newRule, {
  ...
  const handleDeleteRule = async (id) => {
    try {
      await axios.delete(`http://localhost:5004/regex-rules/${id}`);
