...
function ChatCompletion() {
  ...
  useEffect(() => {
    const fetchModels = async () => {
      try {
        const res = await axios.get("http://localhost:5005/models");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
        const res = await axios.post("http://localhost:5005/v1/chat/completions", formData, {
            headers: { "Content-Type": "multipart/form-data" },
        });

        console.log("Response:", res.data);
        
        let botResponse = res.data.response || "No response from model.";
        if (file) {
            botResponse += " (File Processed)";
        }
